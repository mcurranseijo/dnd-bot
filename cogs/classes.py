  ...
  async def classes(self,ctx,args = None,level=None,page = 0,message = None,embed_data = None):
    try:
        ...
        if level:
            ...
            feature_choices = ""
            for choice in data[page]['feature_choices']:
            ...
            if not args:
                embed = discord.Embed(title='D&D Classes',description=f'{count} total classes.',color=Color.red())
                class_names = ""
                for f in classes:
                    class_names+=f'\n*{f["name"]}*'
                ...
                if page <= 0:
                    ...
                    for f in data['saving_throws']:
                        saving_throws+=f'\n*{f["name"]}*'
                    embed = discord.Embed(title=args,description=f'Basic Info, do `>class {args} levels` to view levels ',color=Color.red())
                    fields = [
                    ...
                    try:
                        fields.append(('Spellcasting Ability',data['spellcasting']['spellcasting_ability']['name'],True))
                    except KeyError:
                        pass
                    ...
                    async with self.session.get(f'{BASE_URL}{data["starting_equipment"]}') as resp:
                        equipment = json.loads(await resp.text())['starting_equipment']
                        choices = json.loads(await resp.text())['starting_equipment_options']
